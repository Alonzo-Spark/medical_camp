...
class IntentService:
    ...
    def classify_intent(self, question: str, transcript: str, language: str = 'te') -> tuple:
        ...
        if language == 'hi':
            if question == "Q1":
                # Hindi Q1 keywords (YES)
                yes_keywords = [
                    "हाँ", "करवा लिया", "करवा लिए", "कर लिया", "कर लिए", "हो गया", "करवा दिया", "कर दिया",
                    "हाँ जी", "yes", "done", "completed"
                ]
                for kw in yes_keywords:
                    if kw in transcript_lower:
                        return "YES", 0.95
                
                # Hindi Q1 keywords (NO)
                no_keywords = [
                    "नहीं", "नहीं करवाया", "नहीं किया", "नहीं हुआ", "no", "nahi", "nahin", "nhi"
                ]
                for kw in no_keywords:
                    if kw in transcript_lower:
                        return "NO", 0.95
                
                # Hindi Q1 keywords (PENDING)
                pending_keywords = [
                    "बाकी है", "बचा हुआ है", "कल करूँगा", "कल करवाऊँगा", "कल करेंगे", "कल करवाएँगे", "कल", "pending"
                ]
                for kw in pending_keywords:
                    if kw in transcript_lower:
                        return "PENDING", 0.95

                # Fallback to Gemini for Hindi
                prompt = f"""
                You are an expert medical voicebot assistant.
                Classify the patient's Hindi transcript answering the question: "Did you complete the prescribed lab tests?" (क्या आपने अपने लैब टेस्ट करवा लिए हैं?)
                
                Understand both Hindi Devanagari script, Romanized Hindi (like 'haan', 'nahi', 'kal karenge'), and colloquial variations.
                
                Examples:
                - "हाँ" or "करवा लिया" or "हो गया" or "yes" or "done" -> YES
                - "नहीं" or "नहीं करवाया" or "no" or "nahi" -> NO
                - "कल करवाऊँगा" or "अभी बाकी है" or "कल" or "pending" -> PENDING
                ...
                try:
                    ...
                    print(f"[Intent Hindi] Gemini error: {e}")
            
            elif question == "Q2":
                # Hindi Q2 keywords
                yes_keywords = [
                    "मिल गई", "मिल गया", "ले ली", "ले लिया", "हाँ", "आ गया", "आ गई", "yes", "mil gaya", "le liya"
                ]
                for kw in yes_keywords:
                    if kw in transcript_lower:
                ...
                no_keywords = [
                    "नहीं मिली", "नहीं मिला", "नहीं लिया", "नहीं", "no", "nahi", "nhi"
                ]
                for kw in no_keywords:
                    if kw in transcript_lower:
                ...
                pending_keywords = [
                    "अभी नहीं", "कल मिलेगी", "कल मिलेगा", "waiting", "pending"
                ]
                for kw in pending_keywords:
                    if kw in transcript_lower:
                ...
                prompt = f"""
                You are an expert medical voicebot assistant.
                Classify the patient's Hindi transcript answering the question: "Did you receive your lab reports?" (क्या आपको अपनी लैब रिपोर्ट्स मिल गई हैं?)
                
                Understand both Hindi Devanagari script, Romanized Hindi, and colloquial variations.
                
                Examples:
                - "हाँ मिल गई" or "ले ली है" or "yes" or "received" or "mil gaya" -> REPORT_RECEIVED
                - "नहीं मिली" or "नहीं" or "no" or "nahi" -> REPORT_NOT_RECEIVED
                - "अभी नहीं मिली" or "कल मिलेगी" or "waiting" or "pending" -> REPORT_PENDING
                ...
                try:
                    ...
                    print(f"[Intent Hindi Q2] Gemini error: {e}")
            ...
            if question == "Q1":
                ...
                try:
                    ...
                    print(f"[Intent Telugu] Gemini error: {e}")
                ...
                try:
                    ...
                    print(f"[Intent Telugu Q2] Gemini error: {e}")

        return "UNCLEAR", 0.5

    def classify_multi_test_q1(self, transcript: str, test_names: list, language: str = 'te') -> dict:
        if not test_names:
            return {}
        if not transcript or len(transcript.strip()) < 2:
        ...
        if language == 'hi':
            ...
            The patient was asked: "Did you complete these lab tests?" (क्या आपने ये लैब टेस्ट करवा लिए हैं?)
            
            Analyze the patient's response in Hindi (which may be in Devanagari script, Romanized Hindi, or colloquial mix).
            Determine for each test in the list whether the patient has completed it (YES) or not/pending/no (NO).
            
            Examples:
            1. Patient transcript: "मैने सीबीपी करवा लिया है शुगर अभी बाकी है" (I did CBP, sugar is pending)
               Tests: [CBP, Sugar]
               Output:
               CBP: YES
               Sugar: NO
               
            2. Patient transcript: "सारे करवा लिए हैं" (I did all of them)
               Tests: [CBP, RBS, BP]
               Output:
               CBP: YES
               RBS: YES
            ...
            3. Patient transcript: "कल करवाऊँगा" (Will do tomorrow)
               Tests: [CBP, Sugar]
               Output:
               CBP: NO
               Sugar: NO
        ...
        try:
            ...
            transcript_lower = transcript.lower()
            if language == 'hi':
                yes_keywords = ["हाँ", "करवा लिया", "करवा लिए", "कर लिया", "कर लिए", "हो गया", "yes", "done", "completed"]
            else:
    ...
    def classify_multi_test_q2(self, transcript: str, test_names: list, language: str = 'te') -> dict:
        if not test_names:
            return {}
        if not transcript or len(transcript.strip()) < 2:
            return {name: False for name in test_names}
        ...
        if language == 'hi':
            ...
            The patient was asked: "Did you receive the reports for these tests?" (क्या आपको इन टेस्ट्स की रिपोर्ट्स मिल गई हैं?)
            
            Analyze the patient's response in Hindi (which may be in Devanagari script, Romanized Hindi, or colloquial mix).
            Determine for each test in the list whether the patient has received the report (YES) or not/pending/no (NO).
            
            Examples:
            1. Patient transcript: "सीबीपी मिल गया है शुगर अभी नहीं मिला है" (CBP received, sugar not yet received)
               Tests: [CBP, Sugar]
               Output:
               CBP: YES
               Sugar: NO
               
            2. Patient transcript: "हाँ रिपोर्ट्स मिल गई हैं" (Yes, reports received)
               Tests: [CBP, Sugar]
               Output:
               CBP: YES
               Sugar: YES
        ...
        try:
            ...
            transcript_lower = transcript.lower()
            if language == 'hi':
                yes_keywords = ["हाँ", "मिल गई", "मिल गया", "ले ली", "yes", "received", "mil gaya"]
            else:
